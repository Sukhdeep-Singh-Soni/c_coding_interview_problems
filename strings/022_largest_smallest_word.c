...
void count_words(char *s) {
    ...
    int min_word_start = 0; //start idx of min word
    int min_word_end = 0;  //end idx of min word
    ...
    while (s[i]) {
        if (s[i] == ' ' || s[i] == '\t' || s[i] == '\n') {
            ...
            if (wc < min && word_state == IN) { //IN condition is because min should calculate with at least one character word
                  min = wc;
                  min_word_start = start_idx;
                  min_word_end = end_idx - 1;
            }
            word[wc] = '\0';
            wc = 0;
            word_state = OUT;
    ...
    printf("\n");
    printf("min word is = ");
    for (int i = min_word_start; i <= min_word_end; i++)
        printf("%c", s[i]);
    printf("\n");
}
